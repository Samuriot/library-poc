...
def import_dataset_xlsx(filename: str) -> dict[str, List]:
    ...
    for _, row in dataframe.iterrows():
        ...
        # Skip empty rows
        has_values = any(
            not (pd.isna(value) or (isinstance(value, str) and value.strip() == ""))
        ...
        for item in row_dict:
            if item in ["Time", "Date"]:
                continue
            value = row_dict[item]
            count_value = "-1" if pd.isna(value) else str(value).strip()
            entry_data = {
    ...
    locations_dict: dict[str, List] = {
        location: entries for location, entries in locations.items()
    }
                    ...
                    if len(parts) >= 2:
                        ...
                        for fmt in [
                            ...
                            try:
                                ...
                                entry["datetime"] = (
                                    f"{date_obj.strftime('%Y-%m-%d')} {time_part}"
                                )
                                break
                            except:
                                continue
...
def main():
    if len(sys.argv) < 2:
        print(
            "Exiting program, provide the csv/xlsx file name to parse - E.G. python3 parse.py <FILE_NAME>.csv"
        )
        return
    file_path = Path(sys.argv[1])
    if file_path.suffix not in [".csv", ".xlsx"]:
        print(
            "Exiting program, file provided is not a .csv or .xlsx - E.G. python3 parse.py <FILE_NAME>.csv"
        )
        return
